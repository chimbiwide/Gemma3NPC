...
def create_game_setting(info):
    return f"""You are an expert writer in creating RPG game location descriptions based on NPC biographies.
               You will be given the NPC's biography.
               From the information given, you should write a detailed description around 3-4 sentences
               about the location/environment where the NPC would most naturally be encountered.
               
               The location should:
               - Reflect the NPC's role, occupation, or personality
               - Include some sensory details(sights, sounds, smells, atmosphere)
               - Match the tone and setting implied by the character's background
               - Feel like a natural location for this character.
...
def generate_setting(system_prompt: str):
    ...
    response = client.models.generate_content(
        ...
        config=types.GenerateContentConfig(
            temperature=0.8,  # Increase temperature with each retry
            top_p=0.95,
...
def create_datasets(prompts: List[str], limit: int = None):
    ...
    for i, system_prompt in enumerate(prompts_to_process):
        print(f"  [{i + 1}/{len(prompts_to_process)}] - Generating location description...")
...
def save_datasets(game_dataset: List):
    with open('location_description.txt', 'w', encoding='utf-8') as f:
        for i, setting in enumerate(game_dataset,1):
            f.write(setting)
            f.write("\n")
    print(f"Saved {len(game_dataset)} location descriptions")
...
def main():
    ...
    prompts = read_prompts_txt('char_bio.txt')
    ...
    print(f"Generated {len(NPC_setting)} location description")
